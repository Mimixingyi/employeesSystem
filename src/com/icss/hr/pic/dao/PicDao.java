...
public class PicDao {
	...
	public void insert(Pic pic) throws SQLException {
		...
		pstmt.setBinaryStream(5, pic.getPicData(), (int)pic.getPicSize());// 这个方法必须传入数据流的大小，必须是int           二进制数据流向内赋值（第几个，数据流，文件大小）
		pstmt.setTimestamp(6, new Timestamp(pic.getPicDatetime().getTime()));// 得到毫秒数，自己new一个timestamp，设置时间
